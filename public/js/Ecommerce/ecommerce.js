document.addEventListener("DOMContentLoaded", function () {
    ...
    function calcularValorCarrinho() {
        ...
        document.getElementById(
            "valorTotal"
        ).innerHTML = `<h3>Valor total: R$ ${soma}</h3`;
    }
                ...
                html += `<tr>
                                ...
                                <td>
                                    <div style="display:flex;justify-content:space-evenly;">
                                        ...
                                        <input style="width:70px;" class="form-control" type="text" data-id="${
                                            carrinho[i].id
                                        }" value="${carrinho[i].quantidade}" />
                                        <button  data-id="${
                                            carrinho[i].id + "D"
                                        }" class="btn btn-light "> <i class="fas fa-minus "></i></button>
                                    </div>
                                </td>
        ...
        fetch("/produto/obter/" + produtoId)
            ...
            .then(function (corpo) {
                ...
                if (produtoCarrinho.length == 0) {
                    //nao encontrei, adiciona
                    //inicializa a quantidade com 1;
                    produto.quantidade = 1;
                    carrinho.push(produto);
                } else {
                    //encontrei, incrementa a quantidade
    ...
    for (let i = 0; i < btn.length; i++) {
        btn[i].addEventListener("click", manipularValor);
    }

    function manipularValor() {
        console.log("teste")
    }
});
