...
class ProdutoModel {
    ...
    async gravar() {
        if (this.#produtoId == 0) {
            let sql =
                "insert into tb_produto (prd_cod, prd_nome, prd_quantidade, cat_id, mar_id, prd_imagem, prd_preco) values (?, ?, ?, ?, ?, ?, ?)";
            ...
            //alterar
            let sql =
                "update tb_produto set prd_cod = ?, prd_nome =?, prd_quantidade= ?, cat_id = ?, mar_id = ?, prd_imagem = ?, prd_preco = ? where prd_id = ?";
    ...
    async baixaEstoque(prd_id, nBaixa) {
        const sql = "update tb_produto set prd_quantidade = ? where prd_id = ?";
        const values = [prd_id, nBaixa];
